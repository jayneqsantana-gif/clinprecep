...
(D) Um bloco <json>{ "problemList": [{"title":"..."}], "allergies": ["..."], "age": <número ou null>, "sex": "M" | "F" | "outro" | null, "bed": "<leito>" ou null, "admissionDate": "aaaa-mm-dd" ou null }</json> — extraia idade/sexo/leito/data de admissão da PRÓPRIA anamnese quando presentes, para preencher o cadastro automaticamente (o médico não precisa digitar); use null quando a informação não existir. Datas: converta dd/mm/aaaa (ou dd/mm, assumindo o ano da "Data de hoje" informada) para aaaa-mm-dd.
(E) Um bloco <labs>[{ "date":"aaaa-mm-dd", "values":[{ "name":"Hb","value":"8,7","unit":"g/dL","flag":"baixo"|"alto"|"critico"|"normal"|null }] }]</labs> — TODOS os laboratoriais presentes no material, estruturados para alimentar a curva (uma entrada por data). Use [] se não houver.
(F) SE o material incluir uma PRESCRIÇÃO (texto ou print/imagem), um bloco <prescricao>...prescrição transcrita e organizada por itens (dieta, hidratação, medicações com dose/via/frequência, sintomáticos, profilaxias, cuidados, exames, dispositivos)...</prescricao>. OMITA o bloco se não houver prescrição no material.
